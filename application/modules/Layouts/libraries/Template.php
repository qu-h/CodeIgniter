...
class Template
{
    ...
    /**
     * Set data using a chainable metod.
...
//            }
     ...
     *            of meta data
     * @param string $type
     *            in a few types, links for example
     * @return void
     */
     ...
     * @access public
     * @param string $theme
     *            theme for the template library to use
     * @return void
     */
...
//                 }
    ...
    private function _load_view($view, array $data, $parse_view = TRUE, $override_view_path = NULL)
    {
        if (property_exists($this->_ci, 'smarty')) {
            ...
            if (is_null($file_path)) {
                ...
                foreach (Modules::$locations as $local) {
    ...
    private function _load_smarty_module(){

        if( count(Modules::$registry) > 0 ) foreach (Modules::$registry AS $mod_name=>$module){
            foreach ( Modules::$locations AS $path){
                $module_path = realpath($path.$mod_name.DS."smarty");
                if( $module_path ){
                    $theme_ui_path = realpath($module_path.DS.$this->_theme.".php");
                    if( $theme_ui_path ){
                        include_once $theme_ui_path;
                        $module_ui_class = strtolower($mod_name."_".$this->_theme."_ui");
                        if( class_exists($module_ui_class) ){
                            ...
                            foreach ($methods as $plugin) {
                                ...
                                if ( ! isset($this->_ci->smarty->registered_plugins['function']) or ! array_key_exists($plugin->name, $this->_ci->smarty->registered_plugins['function'])) {
                                    $this->_ci->smarty->registerPlugin('function', $plugin->name, $module_ui_class . '::' . $plugin->name);
                                }
                            }
